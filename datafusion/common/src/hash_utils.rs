...
fn hash_single_array(
    ...
    downcast_primitive_array! {
        ...
        DataType::Struct(_) => {
            ...
            let child_hashing = RandomStateChildHashing {
                random_state,
            };
...
mod tests {
    ...
    #[test]
    #[cfg(not(feature = "force_hash_collisions"))]
    fn test_create_hashes_with_custom_hasher_preserves_signed_zero() {
    ...
    fn test_create_hashes_long_utf8_view_with_custom_hasher() {
        ...
        let expected_hashes = values
            ...
            .map(|value| {
                value
                    .map(|value| hash_builder.hash_one(value))
                    .unwrap_or_default()
            })
            .collect::<Vec<_>>();
        ...
        for (hash, value) in expected_hashes.iter_mut().zip(&values) {
            if let Some(value) = value {
                *hash = combine_hashes(hash_builder.hash_one(value), *hash);
            }
        }
